...
def _invert_points_with_sigma(pts, D_flibe, D_nickel, K_S_nickel):
    ...
    for case_name in sorted({p.case for p in pts}):
        ...
        for p in [pp for pp in pts if pp.case == case_name]:
            ...
            # helper usable for sigma propagation & metrics
            def run_once_safe(
                case,
                T_K,
                P_up,
...
def make_dual_overlay_lnphi(
    ...
    K_S_nickel,
    outdir: Path,
    title_suffix="NORMAL configuration",
    save_csv: bool = True,
    SHOW_JITTER: bool = False,  # keep False to avoid any x-offset
):
    outdir.mkdir(parents=True, exist_ok=True)
    ...
    invT_by, lnphi_by, sig_by, meta_by, metrics_by = _invert_points_with_sigma(  # MOD
    ...
    bc_colors = {
        "normal_infinite": palette[0],
        "normal_transparent": palette[1],
    }
...
if __name__ == "__main__":
    ...
    outdir = Path("exports") / "figs" / "calibration_A" / "NORMAL_bundle"
