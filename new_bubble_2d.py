...
OUTDIR = "plots"
GEOM_LABEL = "fullwidth"

COMM = MPI.COMM_WORLD
RANK = COMM.rank

# NEW MESH
MESH_FILE = "2Dbubble_fullwidth.msh"
...
for temperature in temperatures:
    ...
    if RANK == 0 and len(all_pbs) > 0:
        ...
        csv_pb = os.path.join(
            OUTDIR,
            f"pb_and_fluxdown_{GEOM_LABEL}_{T_label_from_temperature(temperature)}.csv",
        )
        np.savetxt(
            csv_pb,
        ...
        ax_pb.plot(
            t_arr,
            pb_arr,
            label=f"{T_label_from_temperature(temperature)} ({GEOM_LABEL})",
        )
        ax_flux.plot(
            t_arr,
            flux_down_arr,
...
if RANK == 0:
    ...
    fig_pb.tight_layout()
    fig_pb.savefig(
        os.path.join(OUTDIR, f"pb_all_temperatures_{GEOM_LABEL}.png"),
        dpi=200,
    )
    plt.close(fig_pb)
    ...
    fig_flux.savefig(
        os.path.join(OUTDIR, f"flux_down_all_temperatures_{GEOM_LABEL}.png"),
        dpi=200,
    )
    plt.close(fig_flux)
